...
| Главная идея | представление и смысл | хранение/передача данных | дисциплина и совместимость |
...
### 1.5.3. Списки (`ul`, `ol`, `dl`) и вложенные списки
...
### 1.6.2. `rowspan` / `colspan`, группировка колонок, `<tfoot>`
...
### 1.6.3. Семантика и доступность таблиц

* `th` задает заголовки
* можно использовать `scope="col"` / `scope="row"` для ясности
* подпись таблицы — `caption`

### 1.6.4. Пример сложной таблицы (с `caption`, `colgroup`, `thead`, `tbody`, `tfoot`, `rowspan`, `colspan`)
...
### 1.7.1. Структура формы `<form>`, `action`, `method` (GET / POST)
...
### 1.7.3. Валидация: `required`, `pattern`, `minlength`, `maxlength`, `type=email/url`
...
### 1.7.4. Передача данных Web-сервису. Пример формы, работающей с API
...
## 1.10. Доступность (Accessibility)
...
## 1.12. Интеграция HTML с CSS и JavaScript
...
## 1.13. Лучшие практики

* Чистая структура (читаемый, предсказуемый HTML)
* Правильная вложенность (не ломать дерево)
* Валидность (валидатор)
* Отделение структуры (HTML) и представления (CSS)
* Производительность (без лишних узлов, изображения оптимизированы, нет “табличной верстки”)

---

## 1.14. Типичные ошибки новичков

* Неправильная вложенность (например, `p` внутри `p`, интерактив внутри интерактива)
* Использование `div` вместо семантики
* Отсутствие `alt`
* Inline-стили (`style="..."`) вместо CSS
* Дублирование `id` (ломает JS и якоря)
* “Кнопка” через `div` вместо `<button>` (ломает доступность и клавиатуру)
...
### 2.5. Таблица сравнения: block / inline / void / semantic
...
### 2.8. Краткий аналитический вывод (1–2 страницы)
